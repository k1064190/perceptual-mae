...
class Args:
    ...
    def add_core_args(self):
        ...
        self.parser.add_argument(
            "--default_config_path",
            type=str,
            default="./configs/op_default.yaml",
            help="the path to default config",
        )
        self.parser.add_argument(
            "--model_config_path",
            type=str,
            default="./configs/models/operation_t5_dvae_image.yaml",
            help="the path to model config",
        )
        self.parser.add_argument(
            "--dataset_config_path",
            type=str,
            default="./configs/datasets/imagenet1k.yaml",
            help="the path to dataset config",
        )
        self.parser.add_argument(
            "--user_config_path",
            type=str,
            default="./configs/user/op_t5_dvae_sample.yaml",
            help="the path to user config",
        )

        self.parser.add_argument(
            "--model_variant_config_path",
            type=str,
            default=None,
            help="overrides to model config",
        )
        self.parser.add_argument(
            "--experiment_name_from_variant",
            type=bool,
            default=True,
